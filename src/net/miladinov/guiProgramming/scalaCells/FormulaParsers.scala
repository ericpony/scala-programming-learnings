...
object FormulaParsers extends RegexParsers {
  ...
  def application: Parser[Application] = ident ~ "(" ~ repsep(expression, ",") ~ ")" ^^ {
    case f ~ "(" ~ ps ~ ")" => Application(f, ps)
  }

  def expression: Parser[Formula] = range | cell | number | application

  def textual: Parser[Textual] = """[^=].*""".r ^^ Textual

  def formula: Parser[Formula] = number | textual | "=" ~> expression
